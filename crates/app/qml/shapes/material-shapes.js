...
// Notch: flat top (attaches flush to the bar), fully rounded bottom corners
function getNotch() {
    if (_notch !== null) return _notch;
    _notch = barNotch();
    return _notch;
}
...
function getCookie4ExpandedTopLeft() {
    if (_cookie4ExpandedTL !== null) return _cookie4ExpandedTL;
    _cookie4ExpandedTL = cookie4ExpandedTopLeft();
    return _cookie4ExpandedTL;
}

// Bar notch: sharp top-left & top-right, rounded bottom-left & bottom-right
function barNotch() {
    var r = new CornerRounding.CornerRounding(0.4)
    return customPolygon([
        new PointNRound(new Offset.Offset(0.0, 0.0), CornerRounding.Unrounded),  // top-left  (sharp)
        new PointNRound(new Offset.Offset(1.0, 0.0), CornerRounding.Unrounded),  // top-right (sharp)
        new PointNRound(new Offset.Offset(1.0, 1.0), r),                          // bottom-right (rounded)
        new PointNRound(new Offset.Offset(0.0, 1.0), r),                          // bottom-left  (rounded)
    ], 1).normalized();
}
...
// cookie4 with the top-left lobe expanded to ~55% of the shape area.
// Defined directly in 0-1 space (no normalized() needed).
// T and L saddles are shifted toward TR/BL so TL owns more territory;
// TL gets 2× the corner radius for a wide, sweeping arc.
function cookie4ExpandedTopLeft() {
    return customPolygon([
        new PointNRound(new Offset.Offset(1.00, 1.00), new CornerRounding.CornerRounding(0.175)),  // BR peak
        new PointNRound(new Offset.Offset(0.50, 0.82), new CornerRounding.CornerRounding(0.175)),  // B  saddle
        new PointNRound(new Offset.Offset(0.00, 1.00), new CornerRounding.CornerRounding(0.175)),  // BL peak
        new PointNRound(new Offset.Offset(0.28, 0.68), new CornerRounding.CornerRounding(0.175)),  // L  saddle (pushed toward BL)
        new PointNRound(new Offset.Offset(0.00, 0.00), new CornerRounding.CornerRounding(0.38)),   // TL peak (expanded)
        new PointNRound(new Offset.Offset(0.68, 0.28), new CornerRounding.CornerRounding(0.175)),  // T  saddle (pushed toward TR)
        new PointNRound(new Offset.Offset(1.00, 0.00), new CornerRounding.CornerRounding(0.175)),  // TR peak
        new PointNRound(new Offset.Offset(0.82, 0.50), new CornerRounding.CornerRounding(0.175)),  // R  saddle
    ], 1)
}

...
/// Each role falls back to the generated palette's value when the external
/// color file doesn't define it.
fn render_custom_group() -> String {
    let mut out = String::new();
    for (_, qml_name) in ROLES {
        let external_key = external_key_for(qml_name);
        out.push_str(&format!(
            "        readonly property color {qml_name}: root._customColor(\"{external_key}\", root.palette.{qml_name})\n"
        ));
    }
    out
}
...
fn main() {
    ...
    let qml = format!(
        ...
        seed = SEED,
        palette = render_group(&parsed.colors.dark),
